    ...
    <AppShell>
      ...
      <div className="grid gap-3">
        {bookingsQ.data?.length ? bookingsQ.data.map(b => (
            <div key={`${b.class_id}-${b.class_date}`} className="bg-white rounded-xl border p-4 flex items-center justify-between text-slate-900">
              <div>
                <div className="font-semibold">Class #{b.class_id}</div>
                <div className="text-sm text-slate-600">Date: <time dateTime={b.class_date}>{b.class_date}</time></div>
